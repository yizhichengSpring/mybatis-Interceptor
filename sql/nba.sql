...
-- ----------------------------
-- View structure for player_above_avg_height
-- ----------------------------
DROP VIEW IF EXISTS `player_above_avg_height`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `player_above_avg_height` AS select `player`.`player_id` AS `player_id`,`player`.`height` AS `height` from `player` where (`player`.`height` > (select avg(`player`.`height`) from `player`));
